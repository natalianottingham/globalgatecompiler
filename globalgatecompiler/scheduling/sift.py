...
def circuit_to_dag(circuit,draw=False):
    '''
    Inputs:
    ...
    if draw:
        plt.figure()
        pos_nodes = nx.spring_layout(dag)
        nx.draw(dag,pos_nodes,with_labels=True)
        
        positions = {}
        for node, coords in pos_nodes.items():
            positions[node] = (coords[0],coords[1]+0.08)
        
        nx.draw_networkx_labels(dag,positions,labels=node_labels)
