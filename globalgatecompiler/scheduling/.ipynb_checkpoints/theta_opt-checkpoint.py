...
def get_next_dp_args_list(circuit,v_c0,v_p1,v_c1,v_rem,dag_node_to_gate,check_last_cond):
    ...
    qubits_to_check = [circuit.find_bit(dag_node_to_gate[v_c0[0]][1][0]).index]
    for i in range(1,len(v_c0)):
        ...
        if this_op_theta_value!=prev_theta_value:
            ...
            qubits_to_check = []
        qubits_to_check.append(circuit.find_bit(dag_node_to_gate[v_c0[i]][1][0]).index)
    ...
    # starts with largest theta (sorted in reverse order above)
        ...
        for node in v_p1_star:
            node_qubits = set([circuit.find_bit(q).index for q in dag_node_to_gate[node][1]])
...
def dp(circuit,v_p0,v_c0,v_rem,dag_node_to_gate,n,subproblem_graph,subproblem_node_to_moments,
    ...
    args = get_next_dp_args_list(circuit,v_c0,v_p1,v_c1,v_rem,dag_node_to_gate,check_last_cond)
    cost_list = []
    for new_v_c0,new_v_p1,new_v_c1,new_v_rem,new_vc0_cost in args:
        new_node = add_node_to_subproblem_graph(v_p0,new_v_c0,new_vc0_cost,
                                                subproblem_graph,subproblem_node_to_moments,prev_node,node_count)
        cost_list.append(dp(circuit,new_v_p1,new_v_c1,new_v_rem,dag_node_to_gate,n,subproblem_graph,
