...
def get_axial_decomposition_angles(circuit,moment,eta=0):
    '''
...
def get_transverse_decomposition_angles(circuit,moment,eta=0,sign_theta_max=1,sign_sigma_j=1):
    '''
...
def decompose_to_neutral_atom_gate_set(schedule,n,decomposition_type='transverse',use_backlog=True,
    ...
    for moment in schedule:
        ...
        if all([len(op[1])>=2 for op in moment]): # Multi-Qubit Gate Moment (MQGM) --> keep same
            ...
            for op in moment:
                c.append(op[0],qargs=[qr[c.find_bit(qubit).index] for qubit in op[1]])
            decomposed_moments.append(MultiQubitGateMoment([tuple(c.find_bit(qubit).index for qubit in op[1]) for op in moment]))
            ...
            if decomposition_type=='axial':
                decomposition_angles = get_axial_decomposition_angles(c,moment,eta=eta)
            else:
                assert decomposition_type=='transverse', 'Supported decomposition types: axial, transverse'
                decomposition_angles = get_transverse_decomposition_angles(c,moment,eta=eta,
